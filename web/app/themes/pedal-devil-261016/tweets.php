<?php
	...
	**/
	...
	function get_latest_tweets(
		...
		$cache_file          = null, 						 // Change this to the path of your cache file. (Default : null)
		$cache_time          = 600, 						// Seconds to cache feed (Default : 10 minute)
		...
		$date_format         = 'jS M Y'){  			// Date formatting. (http://php.net/manual/en/function.date.php)


		// Initialise Array
		$tweets = array();

		// Initialise cachePath
		// if(empty($cache_file)) $cache_file = realpath(dirname(__FILE__)) . '/cache.txt';
		if(empty($cache_file))
		{
			$upload_dir = wp_upload_dir();
			$cache_file = $upload_dir['basedir'] . '/theme_caches/social/twitter/Twitter.txt';
		}
		...
		if (time() - $cache_time < $cache_file_created) {
			...
			$cache_file_data = file_get_contents($cache_file);
			$tweets = unserialize($cache_file_data);
		} else {
			...
			if($connection)
			{
				...
				if (count($get_tweets)) {
					...
					foreach($get_tweets as $tweet) {
							...
							$tweet_found = true;
							$tweet_count++;
 							$tweet_arr['description'] = $tweet->text;
							// Add hyperlink html tags to any urls, twitter ids or hashtags in the tweet.
							$tweet_arr['description'] = preg_replace("/((http)+(s)?:\/\/[^<>\s]+)/i", "<a href=\"\\0\" target=\"_blank\" class=\"hide\">\\0</a>", $tweet_arr['description'] );
							$tweet_arr['description'] = preg_replace("/[@]+([A-Za-z0-9-_]+)/", "<a href=\"http://twitter.com/\\1\" target=\"_blank\">\\0</a>", $tweet_arr['description'] );
							$tweet_arr['description'] = preg_replace("/[#]+([A-Za-z0-9-_]+)/", "<a href=\"http://twitter.com/search?q=%23\\1\" target=\"_blank\">\\0</a>", $tweet_arr['description'] );


 							// Convert Tweet display time to a UNIX timestamp. Twitter timestamps are in UTC/GMT time.
							$tweet_arr['time'] = strtotime($tweet->created_at);

							// Current UNIX timestamp.
							$current_time = time();
							$time_diff = abs($current_time - $tweet_arr['time']);
							switch ($time_diff)
							{
								case ($time_diff < 60):
									$tweet_arr['formatted_time'] = $time_diff.' seconds ago';
									break;
								case ($time_diff >= 60 && $time_diff < 3600):
									$min = floor($time_diff/60);
									$tweet_arr['formatted_time'] = $min.' minutes ago';
									break;
								case ($time_diff >= 3600 && $time_diff < 86400):
									$hour = floor($time_diff/3600);
									$tweet_arr['formatted_time'] = 'about '.$hour.' hour';
									if ($hour > 1){ $tweet_arr['formatted_time'] .= 's'; }
									$tweet_arr['formatted_time'] .= ' ago';
									break;
								default:
									$tweet_arr['formatted_time'] = date($date_format,$tweet_arr['time']);
									break;
							}

							$tweet_arr['posted_image'] = $tweet->entities->media[0]->media_url_https;

							// Store user data
							$tweet_arr['user_id'] = '@' . $twitter_user_id;
							$tweet_arr['user_image'] = $tweet->user->profile_image_url;
